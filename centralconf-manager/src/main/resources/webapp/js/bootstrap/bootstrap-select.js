				...
				function(jQuery) {
					...
					(function($) {
						...
						if (!String.prototype.includes) {
							(function() {
								'use strict'; // needed to support
												// `apply`/`call` with
												// `undefined`/`null`
								var toString = {}.toString;
								var defineProperty = (function() {
									// IE 8 only supports
									// `Object.defineProperty` on DOM elements
									try {
										var object = {};
								...
								var includes = function(search) {
									...
									// Avoid the `indexOf` call if no match is
									// possible
						...
						if (!Object.keys) {
							...
							r // result array
								...
								for (k in o)
									// fill result array with non-prototypical
									// keys
						...
						// set data-selected on select element if the value has
						// been programmatically selected
						// prior to initialization of bootstrap-select
						// * consider removing or replacing an alternative
						// method *
						var valHooks = {
							useDefault : false,
						...
						$.fn.triggerNative = function(eventName) {
							...
							if (el.dispatchEvent) { // for modern browsers &
													// IE9+
								if (typeof Event === 'function') {
									...
									// For IE since it doesn't support Event
									// constructor
						...
						// Functions for escaping and unescaping strings to/from
						// HTML interpolation.
						var createEscaper = function(map) {
							var escaper = function(match) {
								return map[match];
							};
							// Regexes for identifying a key that needs to be
							// escaped.
						...
						var Selectpicker = function(element, options) {
							// bootstrap-select has been initialized - revert
							// valHooks.select.set back to its original function
							...
							// If we have no title yet, try to pull it from the
							// html title attribute (jQuery doesnt' pick it up
							// as it's not a
							// data-attribute)
						...
						// part of this is duplicated in i18n/defaults-en_US.js.
						...
						Selectpicker.prototype = {
							...
							init : function() {
								...
								// store originalIndex (key) and newIndex
								// (value) in this.liObj for fast accessibility
								// allows us to do
								// this.$lis.eq(that.liObj[index]) instead of
									...
									this.$element.on('invalid', function() {
										...
										that.$element.on({
											...
											'shown.bs.select' : function() {
												that.$element.val(that.$element.val()) // set
																						// the
																						// value
																						// to
																						// hide
							...
							createDropdown : function() {
								// Options
								// If we are multiple or showTick option is set,
								// then add the show-tick class
								...
								var actionsbox = this.multiple && this.options.actionsBox ? '<div class="bs-actionsbox">' + '<div class="btn-group btn-group-sm btn-block">' + '<button type="button" class="actions-btn bs-select-all btn btn-default">' + this.options.selectAllText + '</button>' + '<button type="button" class="actions-btn bs-deselect-all btn btn-default">'
								...
								var drop = '<div class="btn-group bootstrap-select' + showTick + inputGroup + '">' + '<button type="button" class="' + this.options.styleBase + ' dropdown-toggle" data-toggle="dropdown"' + autofocus + ' role="button">' + '<span class="filter-option pull-left"></span>&nbsp;' + '<span class="bs-caret">' + this.options.template.caret + '</span>'
										+ '</button>' + '<div class="dropdown-menu open" role="combobox">' + header + searchbox + actionsbox + '<ul class="dropdown-menu inner" role="listbox" aria-expanded="false">' + '</ul>' + donebutton + '</div>' + '</div>';
							...
							createLi : function() {
								var that = this, _li = [], optID = 0, titleOption = document.createElement('option'), liIndex = -1; // increment
																																	// liIndex
																																	// whenever
																																	// a
																																	// new
								...
								var generateA = function(text, classes, inline, tokens) {
									return '<a tabindex="0"' + (typeof classes !== 'undefined' ? ' class="' + classes + '"' : '') + (inline ? ' style="' + inline + '"' : '') + (that.options.liveSearchNormalize ? ' data-normalized-text="' + normalizeToBase(htmlEscape($(text).html())) + '"' : '')
											+ (typeof tokens !== 'undefined' || tokens !== null ? ' data-tokens="' + tokens + '"' : '') + ' role="option">' + text + '<span class="' + that.options.iconBase + ' ' + that.options.tickIcon + ' check-mark"></span>' + '</a>';
								};

								if (this.options.title && !this.multiple) {
									// this option doesn't create a new <li>
									// element, but does add a new option, so
									// liIndex is decreased
									// since liObj is recalculated on every
									// refresh, liIndex needs to be decreased
									// even if the titleOption is already
									// appended
									liIndex--;

									if (!this.$element.find('.bs-title-option').length) {
										// Use native JS to prepend option
										// (faster)
										...
										// Check if selected or data-selected
										// attribute is already set on an
										// option. If not, select the
										// titleOption option.
										// the selected item may have been
										// changed by user or programmatically
										// before the bootstrap select plugin
										// runs,
										// if so, the select will have the
								...
								this.$element
										...
										.each(
												function(index) {
													...
													// for the option
													var optionClass = this.className || '', inline = this.style.cssText, text = $this.data('content') ? $this.data('content') : $this.html(), tokens = $this.data('tokens') ? $this.data('tokens') : null, subtext = typeof $this.data('subtext') !== 'undefined' ? '<small class="text-muted">' + $this.data('subtext') + '</small>' : '', icon = typeof $this
															.data('icon') !== 'undefined' ? '<span class="' + that.options.iconBase + ' ' + $this.data('icon') + '"></span> ' : '', $parent = $this.parent(), isOptgroup = $parent[0].tagName === 'OPTGROUP', isOptgroupDisabled = isOptgroup && $parent[0].disabled, isDisabled = this.disabled || isOptgroupDisabled;
													...
													if (!$this.data('content')) {
														// Prepend any icon and
														// append any subtext to
														// the main text.
								...
								// If we are not multiple, we don't have a
								// selected item, and we don't have a title,
								// select the first element so something is set
								// in the button
							...
							render : function(updateLi) {
								...
								// Fixes issue in IE10 occurring when no default
								// option is selected and at least one option is
								// disabled
								// Convert all the values into a comma delimited
								// string
								var title = !this.multiple ? selectedItems[0] : selectedItems.join(this.options.multipleSeparator);

								// If this is multi select, and the selectText
								// type is count, the show 1 of 2 selected etc..
								...
								// If we dont have a title, then use the
								// default, or if nothing is set at all, use the
								// not selected text
								if (!title) {
									title = typeof this.options.title !== 'undefined' ? this.options.title : this.options.noneSelectedText;
								}

								// strip all HTML tags and trim the result, then
								// unescape any escaped tags
							...
							liHeight : function(refresh) {
								...
								var newElement = document.createElement('div'), menu = document.createElement('div'), menuInner = document.createElement('ul'), divider = document.createElement('li'), li = document.createElement('li'), a = document.createElement('a'), text = document.createElement('span'), header = this.options.header
										&& this.$menu.find('.popover-title').length > 0 ? this.$menu.find('.popover-title')[0].cloneNode(true) : null, search = this.options.liveSearch ? document.createElement('div') : null, actions = this.options.actionsBox && this.multiple && this.$menu.find('.bs-actionsbox').length > 0 ? this.$menu.find('.bs-actionsbox')[0].cloneNode(true)
										: null, doneButton = this.options.doneButton && this.multiple && this.$menu.find('.bs-donebutton').length > 0 ? this.$menu.find('.bs-donebutton')[0].cloneNode(true) : null;
								...
								if (search) {
									// create a span instead of input as
									// creating an input element is slower
								...
								// fall back to jQuery if getComputedStyle is
								// not supported
							...
							setWidth : function() {
								if (this.options.width === 'auto') {
									...
									// Set width to whatever's larger, button
									// title or longest option
									this.$newElement.css('width', Math.max(ulWidth, btnWidth) + 'px');
								} else if (this.options.width === 'fit') {
									// Remove inline min-width so width can be
									// changed from 'auto'
									...
									// Remove inline min-width/width so width
									// can be changed
									this.$menu.css('min-width', '');
									this.$newElement.css('width', '');
								}
								// Remove fit-width class if width is changed
								// programmatically
							...
							/**
							 * @param {number}
							 *            index - the index of the option that
							 *            is being changed
							 * @param {boolean}
							 *            selected - true if the option is being
							 *            selected, false if being deselected
							 * @param {JQuery}
							 *            $lis - the 'li' element that is being
							 *            modified
							 */
							setSelected : function(index, selected, $lis) {
								if (!$lis) {
									this.togglePlaceholder(); // check if
																// setSelected
																// is being
																// called by
																// changing the
							 ...
							 *            is being disabled
							 * @param {boolean}
							 *            disabled - true if the option is being
							 *            disabled, false if being enabled
							 * @param {JQuery}
							 *            $lis - the 'li' element that is being
							 *            modified
							 */
							...
							clickListener : function() {
								...
								this.$element.on('shown.bs.select', function() {
									...
									// that.options.size === false) return;
									//
								...
								this.$menuInner.on('click', 'li a', function(e) {
									...
									if (!that.isDisabled() && !$this.parent().hasClass('disabled')) {
										...
										} else { // Toggle the one we have
													// chosen if we are multi
													// select.
											...
											if (maxOptions !== false || maxOptionsGrp !== false) {
												...
												if ((maxOptions && maxReached) || (maxOptionsGrp && maxReachedGrp)) {
													if (maxOptions && maxOptions == 1) {
														...
														var maxOptionsText = typeof that.options.maxOptionsText === 'string' ? [ that.options.maxOptionsText, that.options.maxOptionsText ] : that.options.maxOptionsText, maxOptionsArr = typeof maxOptionsText === 'function' ? maxOptionsText(maxOptions, maxOptionsGrp) : maxOptionsText, maxTxt = maxOptionsArr[0].replace('{n}',
																maxOptions), maxTxtGrp = maxOptionsArr[1].replace('{n}', maxOptionsGrp), $notify = $('<div class="notify"></div>');
														// If {var} is set in
														// array, replace it
														/** @deprecated */
														if (maxOptionsArr[2]) {
							...
							keydown : function(e) {
								...
								if (that.options.liveSearch) {
									...
									// $items contains li elements when
									// liveSearch is enabled
								...
								// Select focused option if "Enter", "Spacebar"
								// or "Tab" (when selectOnTab is true) are
								// pressed inside the menu.
								if ((/(13|32)/.test(e.keyCode.toString(10)) || (/(^9$)/.test(e.keyCode.toString(10)) && that.options.selectOnTab)) && isActive) {
									...
									if (!that.options.liveSearch) {
										...
										elem.focus();
										// Prevent screen from scrolling if the
										// user hit the spacebar
										e.preventDefault();
										// Fixes spacebar selection of dropdown
										// items in FF & IE
						...
						function Plugin(option) {
							...
							var args = arguments;
							// The arguments of the function are explicitly
							// re-defined from the argument list, because the
							// shift causes them
							// to get lost/corrupted in android 2.3 and IE9 #715
							// #775
							var _option = option;
